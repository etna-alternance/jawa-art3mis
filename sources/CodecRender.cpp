...
CodecRender::CodecRender(const std::string & filename, const std::string & bgmusic, const std::string & ringtone, const sf::Vector2f & scale)
  : mysf::GraphicRender()
  , _bgmusic(bgmusic)
  , _ringtone(ringtone)
  , _scale(scale)
...
bool CodecRender::init()
{
  ...
  if (initNodes() == false)
    return false;
  if (initTalkers() == false)
    return false;
  if (initLines() == false)
    return false;
  if (_frequency.init(_thl) == false)
    return false;
  return true;
}
...
bool CodecRender::initLines()
{
  for (unsigned int i = 0; i < 2; ++i)
    {
      _lines[i].setTexture(_thl[Resource::Texture::Lines]);
      _lines[i].setPosition(121, 51);
    }
  _lines[0].loop(false);
  _lines[0].setFrameTime(sf::seconds(0.6f / 6.f));
  for (unsigned int i = 0; i < 7; ++i)
    _lines[0].addFrame(sf::IntRect(i * 77, 0, 77, 45));

  _lines[1].setFrameTime(sf::seconds(0.35f));

  for (unsigned int i = 0; i < 6; ++i)
    _lines[1].addFrame(sf::IntRect((6 + (i % 2)) * 77, 0, 77, 45));
  _lines[1].addFrame(sf::IntRect(8 * 77, 0, 77, 45));
  for (unsigned int i = 0; i < 6; ++i)
    _lines[1].addFrame(sf::IntRect((6 + (i % 2)) * 77, 0, 77, 45));
  _lines[1].addFrame(sf::IntRect(8 * 77, 0, 77, 45));
  for (unsigned int i = 0; i < 6; ++i)
  ...
  for (unsigned int i = 9; i >= 6; --i)
    _lines[1].addFrame(sf::IntRect(i * 77, 0, 77, 45));
  return true;
}
...
bool CodecRender::changeTalker()
{
  _talking[_iTalker]->stop();
  _iTalker = _textBox.getIndex();
  _middleground.subChild(*_talking[_iTalker]);

  if ((_talking[_iTalker] = &_talkers[_tbind[_textBox.getTalker()]]) == 0)
    return false;
  _talking[_iTalker]->setPosition(_positions[_iTalker]);

  _middleground.addChild(*_talking[_iTalker]);
  _talking[_iTalker]->play();
  return true;
}
